...
we risk forming a view of reality that is artificially constrained:
...
The specific domain of inquiry determines the level of analysis, and the
analysis is not restricted to only two levels:

| Domain    | Micro          | $\leftarrow$ Macro \| Micro $\rightarrow$ | Macro |
|---------- | :------------: | :-----------------: | :-------------------------: |
...
why [salt
morphology](https://www.sciencedirect.com/science/article/abs/pii/S0963996915300958)
affects the intensity of saltiness, we can measure the dissolution of different
NaCl crystals in artificial saliva.

In **synthesis**, we do a bottom-up evaluation of the macro-level effects. To
explore the relationship between dissolution rate and saltiness, we take salts
with known dissolution rates and get people to evaluate their saltiness.
...
The utility of having a deep explanation of a phenomenon is not always clear.
...
Consider the case of [pink
tea](https://link.springer.com/article/10.1007/s00216-017-0691-1). With some
...
needed to generate the colour, without specifying _why_.
...
explanatory _system_. In addition, the explanation gives us more power to act.
...
releases $OH^-$, with potassium hydroxide ($KOH$) being an obvious candidate.
...
([Maillard](https://en.wikipedia.org/wiki/Maillard_reaction)) browning effect
mentioned earlier can be used to explain everything from the deliciousness of
toast to the [reduced nutritional quality of some
foods](https://link.springer.com/chapter/10.1007/978-1-4757-9113-6_22) during
